...
Lemma exec_alu32_mul_eq_dst_in_ld_set:
  forall l2 dst l1 ld_set st_set ld_set2 st_set2 (rs0 rs1: regset) i jit_blk st_blk m1 ars0 astk0 astkb am
    (Hl1 : l2 ++
      [encode_arm32
         (encode_arm32 (Int.repr 15) (encode_arm32 (int_of_breg dst) (int_of_ireg (ireg_of_breg dst)) 8 4) 12 4)
    ...
    (Hl2 :
      (if set_mem breg_eq dst (set_add breg_eq dst ld_set)
       then []
        ...
        if set_mem ireg_eq (ireg_of_breg dst) arm_callee_save
        then
    ...
    (HALU : Val.mul (rs0 dst) (rs0 dst) = Vint i)
...
Lemma exec_alu32_mul_eq_dst_not_in_ld_set:
  forall l2 dst l1 ld_set st_set ld_set2 st_set2 (rs0 rs1: regset) i jit_blk st_blk m1 ars0 astk0 astkb am
    (Hl1 : l2 ++
      [encode_arm32
         (encode_arm32 (Int.repr 15) (encode_arm32 (int_of_breg dst) (int_of_ireg (ireg_of_breg dst)) 8 4) 12 4)
...
Lemma exec_alu32_mul_neq_dst_in_ld_set:
...
Lemma exec_alu32_mul_neq_dst_not_in_ld_set:
  forall l2 (dst src: breg) l1 ld_set st_set ld_set2 st_set2
    (rs0 rs1: regset) i jit_blk st_blk m1 ars0 astk0 astkb am
    (HALU : Val.mul (rs0 dst) (rs0 src) = Vint i)
    (Hdst_in : set_mem breg_eq dst ld_set = false)
...
Proof.
  ...
  - (**r dst = src *)
    ...
    + (**r dst in ld *)
      ...
      eapply exec_alu32_mul_eq_dst_in_ld_set; eauto.
    + (**r dst not in ld *)
      eapply exec_alu32_mul_eq_dst_not_in_ld_set; eauto.
  - (**r dst <> src *)
    destruct set_mem eqn: Hdst_in in Hl2.
    + (**r dst in ld *)
      rewrite app_nil_l in Hl2.
      eapply exec_alu32_mul_neq_dst_in_ld_set; eauto.
    + (**r dst not in ld *)
      eapply exec_alu32_mul_neq_dst_not_in_ld_set; eauto.
Qed.