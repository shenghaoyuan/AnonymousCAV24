...
From bpf.comm Require Import MemRegion Regs Monad rBPFAST rBPFValues.
...
Section Get_mem_region.
  ...
  Definition match_res : res -> Inv hybrid_state := fun r  => StateFull _ (mr_correct r).
...
Qed.
  ...
  Proof.
    ...
    -
      ...
      assert (HrangeNat: (0 <= c < mrs_num h)%nat) by lia.
      specialize (Hmatch c HrangeNat).
      (**r because 0<=c < length mrs, so we know nth_error must return Some mr *)
      assert (HrangeZ: (0 <= Z.of_nat c < Z.of_nat (mrs_num h))%Z) by lia.
      rewrite <- Hmrs_num_eq in HrangeZ.
      apply memory_region_in_nth_error with (n:=c) (l:= bpf_mrs h) in HrangeZ as Hnth_error.
