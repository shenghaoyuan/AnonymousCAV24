...
From bpf.comm Require Import rBPFAST ListAsArray MemRegion Regs Flag.
...
From bpf.jit.simulation Require Import MatchStateComm HAVMMatchState.

Definition val_ptr_correct {S:special_blocks} (x:val) (v: val) (st: hybrid_state) (m:Memory.Mem.mem) :=
  x = v /\
  match_state st m.
...
Definition opcode_step_correct (op: opcode) (v: val) :=
  ...
  | op_BPF_ILLEGAL_INS =>
    exists i,
      v = Vint (Int.repr (Z.of_nat i)) /\
      (i <> 0x01 /\ i <> 0x02 /\ i <> 0x03 /\ i <> 0x04 /\ i <> 0x05) /\
      i <= 255
  end.
...
Definition mr_correct {S:special_blocks} (mr: memory_region) (v: val) (st: hybrid_state) (m:Memory.Mem.mem) :=
  List.In mr (bpf_mrs st) /\
  match_region st_blk mrs_blk ins_blk mr v st m /\
  match_state  st m.

Definition mrs_correct (S: special_blocks) (mrs: list memory_region) (v: val) (st: hybrid_state) (m:Memory.Mem.mem) :=
