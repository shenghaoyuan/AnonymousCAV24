...
Lemma exec_alu32_sub_r_eq_dst_in_ld_set:
  forall l2 dst l1 ld_set st_set ld_set2 st_set2 (rs0 rs1: regset) i jit_blk st_blk m1 ars0 astk0 astkb am
    (Hl1 : l2 ++
      [encode_arm32 (encode_arm32 (int_of_breg dst) (int_of_ireg (ireg_of_breg dst)) 8 4)
    ...
    (HALU : Val.sub (rs0 dst) (rs0 dst) = Vint i)
...
Lemma exec_alu32_sub_r_eq_dst_not_in_ld_set:
  forall l2 dst l1 ld_set st_set ld_set2 st_set2 (rs0 rs1: regset) i jit_blk st_blk m1 ars0 astk0 astkb am
    (Hl1 : l2 ++
      [encode_arm32 (encode_arm32 (int_of_breg dst) (int_of_ireg (ireg_of_breg dst)) 8 4)
         (encode_arm32 (int_of_breg dst) SUB_R_OP 0 4) 16 16] = l1)
...
Lemma exec_alu32_sub_r_neq_dst_in_ld_set:
      ...
      + unfold match_regs.
        ...
        * split; intro HF.
          { destruct (breg_eq r dst) as [Hr_eq | Hr_neq].
            ...
            { (**r r <> dst /\ r <> src *) destruct HF as (HF & HF1).
              assert (Heq: ~ In r (set_union breg_eq ld_set st_set) /\
                In r rbpf_arm_callee_save). {
                split; auto.
                intro Hin; apply HF.
      ...
      + unfold match_regs.
        ...
        * split; intro HF.
          { destruct (breg_eq r dst) as [Hr_eq | Hr_neq].
            ...
            { (**r r <> dst /\ r <> src *) destruct HF as (HF & HF1).
              assert (Heq: ~ In r (set_union breg_eq ld_set st_set) /\ In r rbpf_arm_callee_save). {
                split; auto.
                intro Hin; apply HF.
                apply set_union_elim in Hin.
                destruct Hin as [Hin | Hin].
...
Lemma exec_alu32_sub_r_neq_dst_not_in_ld_set:
  forall l2 (dst src: breg) l1 ld_set st_set ld_set2 st_set2
    (rs0 rs1: regset) i jit_blk st_blk m1 ars0 astk0 astkb am
    (HALU : Val.sub (rs0 dst) (rs0 src) = Vint i)
    (Hdst_in : set_mem breg_eq dst ld_set = false)
...
Proof.
  ...
  - (**r src not in ld_set *)
    ...
    destruct set_mem eqn: Hcallee.
    {
      ...
      assert (Heq: ~ In src (set_union breg_eq ld_set st_set) /\ In src rbpf_arm_callee_save). {
        split; auto.
        intro HF; apply Hsrc_in.
        apply set_union_elim in HF.
        destruct HF as [HF | HF].
...
Proof.
  ...
  - (**r dst = src *)
    ...
    + (**r dst in ld *)
      ...
      eapply exec_alu32_sub_r_eq_dst_in_ld_set; eauto.
    + (**r dst not in ld *)
      eapply exec_alu32_sub_r_eq_dst_not_in_ld_set; eauto.
  - (**r dst <> src *)
    destruct set_mem eqn: Hdst_in in Hl2.
    + (**r dst in ld *)
      rewrite app_nil_l in Hl2.
      eapply exec_alu32_sub_r_neq_dst_in_ld_set; eauto.
    + (**r dst not in ld *)
      eapply exec_alu32_sub_r_neq_dst_not_in_ld_set; eauto.
Qed.