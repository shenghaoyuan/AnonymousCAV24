...
Lemma exec_alu32_mov_r1_neq_dst_in_ld_set:
  forall (dst: breg) l1 ld_set st_set ld_set2 st_set2
    ...
    (Hhi_0 : decode_arm32 i 0 16 = i)
    (Hl1 : [mov_int_to_movwt i (ireg_of_breg dst) MOVW_OP] = l1)
...
Lemma exec_alu32_mov_r1_neq_dst_not_in_ld_set:
  forall (dst: breg) l1 l2 ld_set st_set ld_set2 st_set2
    (rs0 rs1: regset) i jit_blk st_blk m1 ars0 astk0 astkb am
    ...
    (Hhi_0 : decode_arm32 i 0 16 = i)
    (Hl1 : l2 ++ [mov_int_to_movwt i (ireg_of_breg dst) MOVW_OP] = l1)
    (HST : match_states jit_blk st_blk ld_set st_set (State rs0 m1) (AState ars0 astk0 astkb am))
    (Hrs1 : rs1 =
       BPregmap.set BPC (Val.add (BPregmap.set dst (Vint i) rs0 BPC) Vone)
         (BPregmap.set dst (Vint i) rs0)),
...
Proof.
  ...
  - (**r dst in ld /\ st *)
    ...
    eapply exec_alu32_mov_r1_neq_dst_in_ld_set; eauto.
  - (**r dst not in ld /\ st *)
    eapply exec_alu32_mov_r1_neq_dst_not_in_ld_set; eauto.
Qed.

Lemma exec_alu32_mov_r2_neq_dst_in_ld_set:
  forall (dst: breg) l1 ld_set st_set ld_set2 st_set2
    (rs0 rs1: regset) i jit_blk st_blk m1 ars0 astk0 astkb am
    (Hdst_in : set_mem breg_eq dst (set_union breg_eq ld_set st_set) = true)
    (Hld2 : set_add breg_eq dst ld_set = ld_set2)
    (Hst2 : set_add breg_eq dst st_set = st_set2)
    (Hl1 : [mov_int_to_movwt (decode_arm32 i 0 16) (ireg_of_breg dst) MOVW_OP;
...
Proof.
  ...
  simpl.
  rewrite decode_movw.
  assert (Heq: (Int.unsigned_bitfield_extract 0 16 (decode_arm32 i 0 16)) =
    decode_arm32 i 0 16). {
...
Lemma exec_alu32_mov_r2_neq_dst_not_in_ld_set:
  forall (dst: breg) l1 l2 ld_set st_set ld_set2 st_set2
    (rs0 rs1: regset) i jit_blk st_blk m1 ars0 astk0 astkb am
    (Hdst_in : set_mem breg_eq dst (set_union breg_eq ld_set st_set) = false)
    (Hld2 : set_add breg_eq dst ld_set = ld_set2)
    ...
    (Hl1 : l2 ++
      ...
      mov_int_to_movwt (decode_arm32 i 16 16) (ireg_of_breg dst) MOVT_OP] =
      l1)
    (HST : match_states jit_blk st_blk ld_set st_set (State rs0 m1) (AState ars0 astk0 astkb am))
    (Hrs1 : rs1 =
       BPregmap.set BPC (Val.add (BPregmap.set dst (Vint i) rs0 BPC) Vone)
...
Proof.
  ...
  - (**r dst in ld /\ st *)
    subst l2.
    rewrite app_nil_l in Hl1.
    eapply exec_alu32_mov_r2_neq_dst_in_ld_set; eauto.
  - (**r dst not in ld /\ st *)
    eapply exec_alu32_mov_r2_neq_dst_not_in_ld_set; eauto.
Qed.