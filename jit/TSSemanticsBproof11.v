...
Axiom hardware_support_div: Archi.hardware_idiv tt = true.

Lemma exec_alu32_div_r1_neq_dst_in_ld_set:
...
Proof.
  ...
  simpl.
  rewrite decode_div_r; auto.
  simpl.

  unfold anextinstr. repeat abreg_solver_error_prone.
  rewrite Hdst2 in *.
  simpl.
  rewrite Hzero_0, hardware_support_div.
  rewrite Hdst1 in HALU.
  simpl in HALU.
  rewrite Hzero_0 in HALU.
...
Lemma exec_alu32_div_r1_neq_dst_not_in_ld_set:
...
Proof.
  ...
  { 
    ...
    rewrite decode_div_r.
    simpl.
    unfold anextinstr, aadd; simpl.
    rewrite hardware_support_div.
...
Proof.
  ...
  - (**r dst in ld /\ st *)
    ...
    eapply exec_alu32_div_r1_neq_dst_in_ld_set; eauto.
  - (**r dst not in ld /\ st *)
    eapply exec_alu32_div_r1_neq_dst_not_in_ld_set; eauto.
Qed.

Lemma exec_alu32_div_r2_neq_dst_in_ld_set:
  forall (dst: breg) si l1 ld_set st_set ld_set2 st_set2
    (rs0 rs1: regset) i jit_blk st_blk m1 ars0 astk0 astkb am
    (HALU : Val.divu (rs0 dst) (Vint si) = Some (Vint i))
    (Hdst_in : set_mem breg_eq dst (set_union breg_eq ld_set st_set) = true)
...
Lemma exec_alu32_div_r2_neq_dst_not_in_ld_set:
  forall (dst: breg) si l1 l2 ld_set st_set ld_set2 st_set2
    (rs0 rs1: regset) i jit_blk st_blk m1 ars0 astk0 astkb am
    (HALU : Val.divu (rs0 dst) (Vint si) = Some (Vint i))
    (Hdst_in : set_mem breg_eq dst (set_union breg_eq ld_set st_set) = false)
...
Proof.
  ...
  - (**r dst in ld /\ st *)
    subst l2.
    rewrite app_nil_l in Hl1.
    eapply exec_alu32_div_r2_neq_dst_in_ld_set; eauto.
  - (**r dst not in ld /\ st *)
    eapply exec_alu32_div_r2_neq_dst_not_in_ld_set; eauto.
Qed.