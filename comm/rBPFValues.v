...
Definition comp_eq (x y: val): bool :=
  match x, y with
  | Vint n1, Vint n2 => Int.eq n1 n2
  | _, _ => false
  end.

Definition comp_ne (x y: val): bool :=
  match x, y with
  | Vint n1, Vint n2 => negb (Int.eq n1 n2)
  | _, _ => false
  end.

Definition comp_lt (x y: val): bool :=
  match x, y with
  | Vint n1, Vint n2 => Int.lt n1 n2
  | _, _ => false
  end.

Definition comp_le (x y: val): bool :=
  match x, y with
  | Vint n1, Vint n2 => negb (Int.lt n2 n1)
  | _, _ => false
  end.

Definition comp_gt (x y: val): bool :=
  match x, y with
  | Vint n1, Vint n2 => Int.lt n2 n1
  | _, _ => false
  end.

Definition comp_ge (x y: val): bool :=
  match x, y with
  | Vint n1, Vint n2 => negb (Int.lt n1 n2)
  | _, _ => false
  end.

Definition compu_lt (x y: val): bool :=
  match x, y with
  | Vint n1, Vint n2 => Int.ltu n1 n2
  | _, _ => false
  end.

Definition compu_le (x y: val): bool :=
  match x, y with
  | Vint n1, Vint n2 => negb (Int.ltu n2 n1)
  | _, _ => false
  end.

Definition compu_gt (x y: val): bool :=
  match x, y with
  | Vint n1, Vint n2 => Int.ltu n2 n1
  | _, _ => false
  end.
Definition compu_ge (x y: val): bool :=
  match x, y with
  | Vint n1, Vint n2 => negb (Int.ltu n1 n2)
  | _, _ => false
  end.
Definition compu_set (x y: val): bool :=
  match x, y with
  | Vint n1, Vint n2 => negb (Int.eq (Int.and n1 n2) Int.zero)
  | _, _ => false
  end.
...
Definition int32_to_nat (v: int): nat := Z.to_nat (Int.unsigned v).
Definition nat_to_int32 (n:nat): int := Int.repr (Z.of_nat n).

Definition nat8_to_int (n:nat) := Int.repr (Z.of_nat n).
...
Definition sint32_of_uint32 (i: int): int := i.
