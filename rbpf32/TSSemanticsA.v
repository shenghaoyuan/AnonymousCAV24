...
Inductive astep (c: code) (kl: list (nat * bpf_code)) (ge: genv): state -> trace -> state -> Prop :=
  | aexec_step_alu32:
      forall v n rs m rs' m' l o r ri,
      rs#BPC = Vint (Int.repr (Z.of_nat v)) ->
      find_instr v c = Some (Palu32 o r ri) ->
      List.nth_error kl n = Some (v, l) ->
      (**r big-step *)
      exec_alu32_list l rs m = Next rs' m' ->
      astep c kl ge (State rs m) E0 (State rs' m')

  | aexec_step_jmp_always:
  ...
  | aexec_step_external: (**r call instruction *)
      ...
      astep c kl ge (State rs m) t (State rs' m').
