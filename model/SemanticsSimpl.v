...
From bpf.comm Require Import Flag rBPFValues Regs BinrBPF MemRegion rBPFAST rBPFMemType.
...
Definition step_alu_binary_operation (bop: binOp) (d :reg) (s: reg+imm) (rs: regmap):
  option (regmap * bpf_flag) :=
  let d32 := eval_regmap d rs in
  let s32 := eval_src s rs in
    match bop with
    | BPF_ADD  => Some (upd_regmap d (Val.add  d32 s32) rs, BPF_OK)
    | BPF_SUB  => Some (upd_regmap d (Val.sub  d32 s32) rs, BPF_OK)
    | BPF_MUL  => Some (upd_regmap d (Val.mul  d32 s32) rs, BPF_OK)
    ...
    | BPF_OR   => Some (upd_regmap d (Val.or   d32 s32) rs, BPF_OK)
    | BPF_AND  => Some (upd_regmap d (Val.and  d32 s32) rs, BPF_OK)
    | BPF_LSH  => if compu_lt s32 (Vint (Int.repr 32)) then
                    Some (upd_regmap d (Val.shl d32 s32) rs, BPF_OK)
                  else
                    Some (rs, BPF_ILLEGAL_SHIFT)

    | BPF_RSH  => if compu_lt s32 (Vint (Int.repr 32)) then
                    Some (upd_regmap d (Val.shru d32 s32) rs, BPF_OK)
                  else
                    Some (rs, BPF_ILLEGAL_SHIFT)
    ...
    | BPF_MOD  => if comp_ne s32 Vzero then (** run-time checking *)
                    ...
                    Some (rs, BPF_ILLEGAL_DIV)
    | BPF_XOR  => Some (upd_regmap d (Val.xor  d32 s32) rs, BPF_OK)
    | BPF_MOV  => Some (upd_regmap d s32 rs, BPF_OK)
    | BPF_ARSH => if compu_lt s32 (Vint (Int.repr 32)) then
                    Some (upd_regmap d (Val.shr  d32 s32) rs, BPF_OK)
                  else
                    Some (rs, BPF_ILLEGAL_SHIFT)
    end.
...
Definition step (pc: int) (l: list int64) (len: nat) (rs: regmap) (mrs_num: nat) (mrs: MyMemRegionsType) (m: mem):
  ...
  | Some ins64 =>
    ...
    | Some ins =>
      ...
      | BPF_NEG d =>
        let dv := eval_regmap d rs in
          Some (pc, upd_regmap d (Val.neg dv) rs, m, BPF_OK)
      | BPF_BINARY bop d s =>
        match step_alu_binary_operation bop d s rs with
        | None => None
...
Fixpoint bpf_interpreter_aux (fuel: nat) (pc: int) (l: list int64) (len: nat)
  ...
  | S fuel0 =>
    if Int.cmpu Clt pc (Int.repr (Z.of_nat len)) then
      match step pc l len rs mrs_num mrs m with
      | None => None
      | Some (pc', rs', m' ,f') =>
        if flag_eq f' BPF_OK then
          if Int.cmpu Clt (Int.add pc' Int.one) (Int.repr (Z.of_nat len)) then
            bpf_interpreter_aux fuel0 (Int.add pc' Int.one) l len rs' mrs_num mrs m'
          else
...
Definition bpf_interpreter (fuel: nat) (ctx_ptr: val) (pc: int) (l: list int64) (len: nat)
  (rs: regmap) (mrs_num: nat) (mrs: MyMemRegionsType) (m: mem): option (val * int * regmap * mem * bpf_flag) :=
  let rs1 := upd_regmap R1 ctx_ptr rs in
    match bpf_interpreter_aux fuel pc l len rs1 mrs_num mrs m with
    | None => None
    | Some (pc', rs', m' ,f') =>
