...
Definition step_alu_binary_operation (bop: binOp) (d :reg) (s: reg+imm): M state unit :=
  do d32 <- eval_reg d;
  do s32 <- eval_src s; (**r (u32) DST, (u32) SRC/IMM *)
  match bop with
  | BPF_ADD  => upd_reg d (Val.add  d32 s32)
  | BPF_SUB  => upd_reg d (Val.sub  d32 s32)
  | BPF_MUL  => upd_reg d (Val.mul  d32 s32)
  | BPF_DIV  => if comp_ne s32 Vzero then (** run-time checking *)
  ...
  | BPF_OR   => upd_reg d (Val.or   d32 s32)
  | BPF_AND  => upd_reg d (Val.and  d32 s32)
  | BPF_LSH  => if compu_lt s32 (Vint (Int.repr 32)) then
                  upd_reg d (Val.shl d32 s32)
                else
  ...
  | BPF_MOD  => if comp_ne s32 Vzero then (** run-time checking *)
                  ...
                  upd_flag BPF_ILLEGAL_DIV
  | BPF_XOR  => upd_reg d (Val.xor  d32 s32)
  | BPF_MOV  => upd_reg d s32
  | BPF_ARSH => if compu_lt s32 (Vint (Int.repr 32)) then
                  upd_reg d (Val.shr  d32 s32)
                else
...
Definition step : M state unit :=
  ...
  do ins  <- decodeM ins64;
    ...
    | BPF_NEG d =>
      do d32 <- eval_reg d;
         upd_reg d (Val.neg d32)

    | BPF_BINARY bop d s =>
      step_alu_binary_operation bop d s
